...
admin_site1<-admin_site1 %>% 
  mutate(admission_timestamp = paste0(admission.date, ' ',admission.time),
...
#Question 2
admin<-admin %>% 
  mutate(length_of_stay = discharge_timestamp - admission_timestamp,
...
#Question 5
lab_wide<-question3 %>% 
  pivot_wider(names_from = test_name, 
...
#Question 7
lab_impute1<-lab %>% 
  mutate(result_year = lubridate::year(result_date),
...
lab_impute5<-inner_join(lab_impute4, lab_impute2, by = 'test_name') %>% 
...
year_output<-function(inputted_year, output_file_name){
  ...
  write.csv(ds, paste0('output/',output_file_name,'_',inputted_year,'.csv'))
 # return(ds)
}
